...
X = trials[:, :, start_index + 1:]  # keep only after imagined speech onset
...
print("Total training trials (30/class):", train_idx_all.size)  # 13*30 = 390
print("Total test trials (5/class):", test_idx_all.size)        # 13*5 = 65
...
per_class_acc = per_class_correct / per_class_counts  # shape: (n_classes,)
...
# # Plot 
# weights = [letter_weights.get(word[0].upper(), 0) for word in idx_to_word.values()]
# accuracies = per_class_acc * 100  # Convert to percentage
# weighted_accuracies = [weight * acc for weight, acc in zip(weights, per_class_acc)]
# fig, ax1 = plt.subplots(figsize=(10, 6))
# ax1.bar(range(n_classes), weights, alpha=0.6, color='b', label='Class Weights')
# ax1.set_xlabel('Class')
# ax1.set_ylabel('Weights', color='b')
# ax1.tick_params(axis='y', labelcolor='b')
# ax2 = ax1.twinx()
# ax2.plot(range(n_classes), weighted_accuracies, color='r', marker='o', label='Weighted Accuracy')
# ax2.set_ylabel('Weighted Accuracy (%)', color='r')
# ax2.tick_params(axis='y', labelcolor='r')
# plt.title('Class Weights and Weighted Accuracies')
# plt.xticks(range(n_classes), [idx_to_word[i] for i in range(n_classes)], rotation=45)
# plt.tight_layout()
# plt.show()
