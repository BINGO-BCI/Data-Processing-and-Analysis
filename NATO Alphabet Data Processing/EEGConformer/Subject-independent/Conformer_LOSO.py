...
for test_subject in subjects:
    ...
    # Band-pass filter the data (optional)
    ...
    model =  EEGConformer(n_outputs=n_classes, n_chans=n_chans, n_times=int(input_window_seconds*sfreq), sfreq=sfreq, 
                          n_filters_time=40, filter_time_length=25, pool_time_length=75, pool_time_stride=15, 
    ...
    clf = EEGClassifier(model,criterion=nn.CrossEntropyLoss, optimizer=torch.optim.AdamW,
        batch_size=64, train_split=ValidSplit(0.2),callbacks=["accuracy", EarlyStopping(monitor="valid_loss", patience=10)],
        device='cuda' if torch.cuda.is_available() else 'cpu', classes=list(range(n_classes)), max_epochs=200 )
